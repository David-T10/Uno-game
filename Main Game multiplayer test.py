...
class Player2(Player):
    def discard(self):
        ...
        print("{} discarding card".format(self.name))
        down = 1
        multiplayergame.p2played = True
        if len(self.hand) != 0:
            discard_card = self.hand[down]
            self.throwAway(discard_card) #if the player has cards left in their hand, it will discard the card based on their keyboard input using throwAway
            multiplayergame.p2played = True
        return multiplayergame.p2played
...
mdeck = Deck()
mdeck.shuffle()
mPlayer1 = Player("Player1")
mPlayer2 = Player2("Player2")
mPlayer1.draw(mdeck , 5)
mPlayer2.draw(mdeck , 5)
...
class maingame: #class for main game functionality, OOP required for multiplayer purposes
    ...
    def startup_menu():
        ...
        while startup:
            ...
            mutliplayer_button = maingame.createbutton('MULTILPLAYER',250,450,160,50,blue,orange,multiplayergame.multiplayer_gui)
...
maingame.uno_gui()
#multiplayergame.multiplayer_gui()
